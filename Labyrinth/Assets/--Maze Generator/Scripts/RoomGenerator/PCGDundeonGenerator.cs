...
public class PCGDundeonGenerator : MonoBehaviour {
    ...
    public void Generate () {
        ...
        StartCoroutine (SpawningRooms (dungeon.startRoom, new Vector3 (0, 0, 0), null)); //Start generating dungeon
    }
    ...
    IEnumerator SpawningRooms (RoomDataObject roomData, Vector3 roomPosition, RoomDataObject _previousRoom) {
        ...
        if (currentInbetweenRooms >= -1) {
            ...
            yield return new WaitForSeconds (dungeon.waitTime);
            /**/
            currentInbetweenRooms--;
            if (currentInbetweenRooms >= -1) {
                //restart coroutine whilst there are inbetween rooms
                StartCoroutine (SpawningRooms (nextRoom, nextRoomPosition, currentRoom));
            }
        }
    }
